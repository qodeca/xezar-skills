...
| 24 | The onboarding kit's workflows and role skills load under the kit's own validator, every workflow has a row in `kit/routing.json`, the file passes `route.mjs --check` and matches its schema and stored defaults, `route` answers right on a staged project, the maintained-skill list is the skill directory, and every row and class count in prose is the file's | `test-kit-catalog.mjs` | ✅ — proves a workflow loads and can be selected, and runs `config-guard.sh` and `deploy-guard.sh` for real on a throwaway repository; **not** that a workflow runs on an engine |
...
| 26 | A reading step's shell is limited to reading commands and two write helpers, and loosening that is a trust-boundary change | `catalog-check.mjs` reader rule, `test-kit-facts.mjs` FACT 16, seven break cases in `test-guards.mjs` | ✅ — proves the lists; **not** that a runner honours them. Claude enforces an allowlist, Codex ignores it (confined only), pi drops its shell under one |
| 27 | Routing keeps its security minimums and reserved lanes, reads from the base branch, and is a trust boundary | `route.mjs --check` in `test-kit-catalog.mjs`, `test-kit-facts.mjs` FACT 17, twelve break cases in `test-guards.mjs` | ✅ — proves the file and the script; **not** which lanes are available on a machine, which only the leader's lane cache knows |
