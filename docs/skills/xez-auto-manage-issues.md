...
Works on tracker issues (not PRs), so it emits no `PR:` chaining reference lines except the spec-PR link when `--write-missing-specs` authors one, and routes a labelled, prepped issue onward to [xez-auto-fix-issue](xez-auto-fix-issue.md). It delegates implementation-prep to [xez-root-cause](xez-root-cause.md) when installed and, under `--write-missing-specs`, to [xez-auto-write-spec](xez-auto-write-spec.md); [xez-prepare-issue](xez-prepare-issue.md) and [xez-spec-writing](xez-spec-writing.md) cover the create-new and deep-design paths it deliberately does not.

---
*Source: [`skills/xez-auto-manage-issues/SKILL.md`](../../skills/xez-auto-manage-issues/SKILL.md)*
