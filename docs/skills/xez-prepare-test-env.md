...
| `--browser-provider <name>` | Optional | Override `browser.provider` for this generation; matching `.xezar/pipeline/browsers/<name>.md` must exist. |
...
Emits the shared environment descriptor `.local/qa/test-env.json` (base URL, services, start scripts) that consumer skills such as [xez-integration-tests](xez-integration-tests.md) and the auto QA flow attach to, plus the generated up/down scripts under `.xezar/pipeline/scripts`. It provisions the browser provider defined by the pipeline set up in [xez-setup-agent-pipeline](xez-setup-agent-pipeline.md).

---
*Source: [`skills/xez-prepare-test-env/SKILL.md`](../../skills/xez-prepare-test-env/SKILL.md)*
