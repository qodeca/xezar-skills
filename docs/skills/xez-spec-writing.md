...
Writes specs into the configured specs directory (`paths.specs`, default `.xezar/pipeline/specs`) using the `{YYYY-MM-DD}-{kebab-case-title}.md` filename shape. Its phased implementation breakdown maps directly onto [xez-auto-create-pr](xez-auto-create-pr.md)'s execution plan (spec referenced as `Source doc:`), and once a spec ships as a PR [xez-followup-issue-from-pr](xez-followup-issue-from-pr.md) can file the `Implement:` tracking issue; `--autonomous` runs are driven by an `xez-auto-*` caller.

---
*Source: [`skills/xez-spec-writing/SKILL.md`](../../skills/xez-spec-writing/SKILL.md)*
