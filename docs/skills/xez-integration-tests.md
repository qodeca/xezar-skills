...
Attaches to the shared test environment produced by [xez-prepare-test-env](xez-prepare-test-env.md) (the `<paths.qa>/test-env.json` descriptor), invoking that skill to discover or provision one when none is valid. It reads specs from the repo's design-doc area (e.g. those written via [xez-spec-writing](xez-spec-writing.md)) or recent changes to decide what to test, and reports failures classified by owner rather than modifying source.

---
*Source: [`skills/xez-integration-tests/SKILL.md`](../../skills/xez-integration-tests/SKILL.md)*
