...
In PR mode consumes a `{prNumber}` (the `PR:` reference line a PR-producing skill emitted), posts screenshot QA evidence back to that PR, and ends with the `PR:` reference line (plus `Issue:` when known); in local mode the artifacts folder is the deliverable. Its review-first gate runs [xez-auto-review-pr](xez-auto-review-pr.md) when the PR is unreviewed, it boots the app via [xez-prepare-test-env](xez-prepare-test-env.md), records a follow-up scenario for [xez-integration-tests](xez-integration-tests.md), and defers a missing browser provider to [xez-setup-agent-pipeline](xez-setup-agent-pipeline.md).

---
*Source: [`skills/xez-auto-qa-pr/SKILL.md`](../../skills/xez-auto-qa-pr/SKILL.md)*
