...
| `--dry-run` | Optional | Print the drafted entry to stdout only — do not edit `CHANGELOG.md` and do not invoke `xez-auto-create-pr`. |
| `--slug <kebab-case>` | Optional | Override the slug passed to `xez-auto-create-pr`. Defaults to `changelog-<version>`. |

## Works with

Consumes a window of merged PRs and delegates all PR mechanics to [xez-auto-create-pr](xez-auto-create-pr.md) (required — the run stops if it is missing), which opens the docs PR and emits the `PR:` chaining reference line this skill surfaces in its report. It pairs well with [xez-close-fixed-issues](xez-close-fixed-issues.md), which processes the same PR window but mutates the issue tracker instead.

---
*Source: [`skills/xez-auto-update-changelog/SKILL.md`](../../skills/xez-auto-update-changelog/SKILL.md)*
