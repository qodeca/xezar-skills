...
```markdown
You are an executor for xez-auto-create-pr-loop run {SLUG}. Implement exactly one Step.
...
Sibling auto-skills (`xez-auto-continue-pr-loop`, `xez-auto-update-changelog`) inherit this pattern when driving multiple Steps in a single invocation.
