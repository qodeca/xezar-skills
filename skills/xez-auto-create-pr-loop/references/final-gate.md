...
- Run the repo's integration suite via the `xez-integration-tests` skill (running-only mode), full scope. Capture the report summary and save it as `final-gate-artifacts/integration-report-summary.log`. On failure, fix forward with new Steps; never skip a failing suite.

**Design-system / style compliance pass** — after the above are green: when the repo has a design-system or style compliance skill or lint (a repo-local override file under `.xezar/pipeline/overrides/`, or a configured command), run it over the branch diff of this run (`origin/$BASE_BRANCH..HEAD`):
