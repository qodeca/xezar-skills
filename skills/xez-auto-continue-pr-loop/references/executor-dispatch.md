...
```markdown
You are an executor for xez-auto-continue-pr-loop PR #{prNumber}. Implement exactly one Step.
...
The creator counterpart (`xez-auto-create-pr-loop`) inherits this pattern when driving multiple Steps in a single invocation.
