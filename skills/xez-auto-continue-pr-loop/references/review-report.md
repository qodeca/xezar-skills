...
Detailed procedure for the single automated review loop of `xez-auto-continue-pr-loop`.

## Automated review with `xez-auto-review-pr`

Before you post the final summary comment, push the final changes, or flip the PR body to `complete`, subject the resumed PR to its authoritative review with the `xez-auto-review-pr` skill. Do not run a separate direct `xez-code-review` pass first: `xez-auto-review-pr` invokes that engine verbatim and applies compatibility, security, contract, breaking-change, and scope checks.
...
3. Loop, **at most twice**, until `xez-auto-review-pr` returns a clean verdict or the remaining findings are non-actionable (out-of-scope, false positive) and explicitly documented in the summary comment you post in step 9.
...
- The claim check inside `xez-auto-review-pr` will recognize that the current user already owns the `in-progress` lock (claimed in step 1), so it proceeds as re-entry without re-claiming.
