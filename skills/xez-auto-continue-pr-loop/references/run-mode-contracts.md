...
- Still respect: an isolated worktree (skip straight to step 4 for worktree setup); the three-signal `in-progress` lock (already claimed in step 1, released at run end); label discipline (pipeline + category + meta); the single step-8 `xez-auto-review-pr` pass in autofix mode (compatibility checks inside it).
...
Keep the full contract documented in the rest of the `SKILL.md` file: run-folder lookup, HANDOFF.md → Tasks table → NOTIFY tail orientation, lean per-Step commits, checkpoint-batched verification, full validation gate before flipping to `complete`, `xez-auto-review-pr` autofix pass, outcome and handoff comment using `references/summary-comment-template.md`.
