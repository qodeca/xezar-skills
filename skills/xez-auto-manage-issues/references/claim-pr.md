...
## xez-auto-manage-issues specifics
...
- The only claiming in this skill's orbit happens inside the delegated `xez-auto-write-spec {issueId}` run (under `--write-missing-specs`): that skill claims the issue, opens the design-only spec PR, and owns its own release — do not second-guess it.
