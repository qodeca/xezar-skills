...
The step-2.4 procedure `xez-auto-manage-issues` runs to prepare an issue for
implementation: a **read-only** root-cause / impact analysis posted as a comment so
the next agent (`xez-auto-fix-issue`) or a human can start fixing without
...
- **Bug / defect** — when `xez-root-cause` is installed, invoke it verbatim
  (`xez-root-cause {issueId}`); it is read-only and returns evidence, likely cause,
...
- **Feature, or `xez-root-cause` not installed** — do a lighter inline analysis
  ...
  the same shape of guidance `xez-prepare-issue` writes for a new issue — reuse that
...
When `${SPECS_DIR}/product-brief.md` exists (written by `xez-discover`), end the
...
```markdown
🤖 `xez-auto-manage-issues` — implementation notes
...
Next: `xez-auto-fix-issue {issueId}`.
```

Recognize the legacy `` 🤖 `xez-auto-manage-issues` implementation notes — ``
