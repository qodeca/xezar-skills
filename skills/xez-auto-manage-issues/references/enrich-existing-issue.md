...
The per-issue procedure `xez-auto-manage-issues` runs in step 2, for the single id
...
`` 🤖 `xez-auto-manage-issues` — 🏷️ label rationale `` covering the applied set,
...
covering spec exists — the same approach as `xez-prepare-issue` step 2, read-only:
...
```markdown
🤖 `xez-auto-manage-issues` — spec required
...
Link a design here, or run `xez-auto-write-spec {issueId}` to draft one.
```
...
**`xez-auto-write-spec {issueId}`** verbatim — it claims the issue, writes the spec
via `xez-spec-writing --autonomous`, opens the spec PR (`Refs #{issueId}`), and emits
...
```markdown
🤖 `xez-auto-manage-issues` — not ready
...
⛔ `xez-auto-fix-issue` will not implement a feature until these are on the ticket. Add them here, or write "ready as is" to waive.
```
