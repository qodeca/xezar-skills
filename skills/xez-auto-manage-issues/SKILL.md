...
It is the read-write counterpart to `xez-prepare-issue` (which files new issues):
...
deep design work hand off to `xez-spec-writing`; to implement, hand off to
`xez-auto-fix-issue` (it handles both bugs and features).
...
- Existing issues only: this skill never creates an issue (that is `xez-prepare-issue`) and never edits repository source files. It mutates only labels, issue bodies, and comments — the implementation-prep analysis and the spec-coverage check are strictly **read-only** on the codebase. The single exception is `--write-missing-specs`, which delegates to `xez-auto-write-spec` to open a **design-only** spec PR (never implementation).
