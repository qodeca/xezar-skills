...
- `toolchain.providers` — optional **list** of dependency-lifecycle providers, each selecting `.xezar/pipeline/toolchains/<name>.md`. A list because one repository often has several ecosystems. Empty or absent means no toolchain operation applies — `not-applicable`, never `unknown`. Shipped: `npm`, `yarn` (Yarn 1 only), `cargo`, `dotnet`; scaffold others from `references/toolchains/TEMPLATE.md`.
- `dependencies.units` — optional; read only by the `xez-onboard-opinionated` kit, and always from the base branch. For a repository with no single root manifest: the list of folders the kit installs, each `{dir, provider, lockfile?, entry?}` — `provider` is `npm`, `yarn` (Yarn 1) or `dotnet`; `lockfile` picks the file when a folder has two (`yarn.lock`, `package-lock.json`); `entry` is the `.sln` or `.slnx` a dotnet unit restores. Absent means a single npm root, unchanged. When set, `validation.commands[0]` is the kit's `deps-restore.sh` install entry point. An empty list is refused; so is a folder with a symlinked component. A folder left out is never installed.
