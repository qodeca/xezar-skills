...
This is the split-provider implementation of the tracker operations contract for teams that keep issues in Jira Cloud and source code on GitHub. Issue operations use Atlassian CLI (`acli`); repository, pull-request, review, CI, and PR-label operations delegate to the companion `.xezar/pipeline/trackers/github.md` descriptor.

At runtime, `xez-setup-agent-pipeline` installs both files and sets `"tracker": "jira"`. The repository's copies are authoritative: teams may add fields, Jira workflow mappings, or project conventions without editing installed skills.
...
- Install and authenticate the `gh` CLI, and keep the companion `.xezar/pipeline/trackers/github.md` descriptor. Pull-request and CI operations fail loudly when either is missing.
...
atlassian_tracker_auth_check() {
  ...
  [ -f .xezar/pipeline/trackers/github.md ] || {
    echo "Missing companion .xezar/pipeline/trackers/github.md" >&2
    return 1
  }
  gh auth status
}
...
For an issue claim, use the configured Jira account id. For a PR claim, execute **current-user** from `.xezar/pipeline/trackers/github.md` instead.
...
Repository identity comes from **repo-info** in `.xezar/pipeline/trackers/github.md`. Jira issue scope is `${ATLASSIAN_SITE}/${ATLASSIAN_PROJECT}` (or the explicit project key passed as `{repo}`); never silently substitute a GitHub repository name for a Jira project key.

#### default-branch

Execute **default-branch** from `.xezar/pipeline/trackers/github.md`.
...
Every operation in this section executes the same-named operation from `.xezar/pipeline/trackers/github.md`, preserving its inputs, outputs, guards, pagination, and cross-repository rules.
