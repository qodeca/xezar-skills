...
7. **Generate authorized guidance.** Follow `references/project-docs.md`. Select relevant content and paths from the project's own conventions and the user's requested outputs. Show each proposed document before writing unless the exact write is already authorized. Preserve existing files and link them instead of duplicating their rules.
