...
- **Review** for an existing concept, flow, design, prototype, or product area
  (including a whole module handed over by `xez-ux-review-pr`).
...
**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-ux-shape.md` when present; safety rules still win.
...
- If the user asks to build the feature, use this workflow to decide, then
  ...
  collection's implementing skills; `xez-ux-review-pr` closes the loop on the
  resulting PR.
