...
| Root `.gitignore` | Add `.local/` once before local evidence/state/backups; verify the entire directory is ignored, without exceptions. Report already tracked local state, never silently untrack it. Preserve other ignore rules. If existing exceptions prevent full ignoring, preview the exact fix and stop dependent local writes until resolved. |
...
4. Keep secret-free recovery originals and the per-file application journal in the task's ignored evidence directory. For non-Git work this is required before overwriting. If a target contains credentials, do not copy it into evidence; stop that dependent edit and report the limitation.
