...
The one procedure for turning a `{spec}` reference into a concrete spec file. Shared contract: `xez-auto-create-pr --spec` and `xez-auto-fix-issue`'s feature-route spec lookup follow this same order — update all consumers together.
