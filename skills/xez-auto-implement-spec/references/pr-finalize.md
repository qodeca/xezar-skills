...
The single procedure for the "commit → push → open (or reuse) the PR → normalize labels → summary comment → chaining reference lines" mechanics. The point is **one** implementation of PR opening + labeling, reused rather than copied, and never a second PR for work that already has one. In `xez-auto-implement-spec` these mechanics run **inside the engine skills** (`xez-auto-create-pr`, `xez-auto-continue-pr`/`-loop`, and through them `xez-open-pr`); this file is the contract this skill routes by (step 2) and confirms at the end (step 4).
...
`xez-open-pr` is an **optional** enhancement — a repo may run this pipeline without it, and it must still work. When `xez-open-pr` is absent, the engine performs the mechanics inline:

1. Commit the worktree changes with a conventional-commit subject; push the branch.
2. Open the PR via the tracker operation **create-pr** against `$BASE_BRANCH`, with the engine's unified PR-body template (its mandatory `Tracking plan:` line is what lets `xez-auto-continue-pr` resume).
3. Normalize labels per the section below.

Behavior is identical either way — the same PR, the same labels — so installing `xez-open-pr` only removes duplication, it never changes the outcome.
