...
The route `xez-auto-fix-issue` takes (instead of the bug chain, steps 4–12) when
...
`xez-auto-continue-pr {prNumber}` — **unless** that PR is a **spec-only design PR**
...
b. **Spec found** (a path, or the spec-only `SPEC_PR` from F2) → invoke
   **`xez-auto-implement-spec {SPEC_PATH-or-SPEC_PR} [--no-ui] [--loop] [--force]`** verbatim.
   Include `--loop` **only when the user passed it to `xez-auto-fix-issue`** — never
   add it on your own; without it the plain engines are the default and
   `xez-auto-create-pr` escalates to the loop on its own when the drafted plan
...
c. **No spec** → invoke **`xez-auto-write-spec {issueId} [--slug …] [--force]`**
   ...
   **`xez-auto-implement-spec {SPEC_PATH}`**, which opens the **implementation PR**
   (`Refs` the spec PR, `Closes #{issueId}`) — the spec PR stays design-only. For a
   spec **without** implementation, users run `xez-auto-write-spec` directly.
...
- The **full label set** is present — pipeline `review`/current state, `feature`
  ...
  `references/pr-finalize.md` (the `xez-open-pr` step-6 contract) on anything
  missing.
