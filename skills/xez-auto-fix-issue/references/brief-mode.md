...
The path `xez-auto-fix-issue` takes when its required argument is a free-form
...
1. **Invoke the `xez-prepare-issue` skill verbatim** with the description as its
   ...
   duplicate. When `xez-prepare-issue` is not installed, stop and name it as the
   missing chain skill.

2. **Run it under this chain's autonomous contract.** `xez-prepare-issue` is an
...
4. **Respect an authored spec PR.** When `xez-prepare-issue` took its
...
`xez-auto-write-spec` under the same automation identity is re-entry), and
step 2 classifies it — `xez-prepare-issue`'s category label feeds the
