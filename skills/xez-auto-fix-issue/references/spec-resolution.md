...
The one procedure for turning a `{spec}` reference into a concrete spec file — here `{spec}` is the issue id. Shared contract: `xez-auto-create-pr --spec` and `xez-auto-implement-spec` follow this same order — update all consumers together.
...
Validate the resolved file: it must contain an `## Implementation Plan` (or `## Phasing`) section to be implementable. When it lacks one, stop with the not-found notification variant: "spec found but has no implementation breakdown — run xez-spec-writing to complete it first."
...
- {path} — {title}
- …
Next: pass an exact path, or create the spec first with xez-auto-write-spec "{spec}".
```

## xez-auto-fix-issue specifics

On this skill's feature route "not found" is not a stop: it routes to F3c — invoke `xez-auto-write-spec {issueId}` to author the spec first, then chain `xez-auto-implement-spec {SPEC_PATH}`.
