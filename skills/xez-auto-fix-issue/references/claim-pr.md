...
- **Step 1 only decides.** The actual claim (assignee + `in-progress` label + claim comment) happens inside `xez-fix` on the bug route, after triage confirms there is real work to do — so a stopped chain never leaves a stray lock behind. On the feature route the delegated skills (`xez-auto-write-spec` / `xez-auto-implement-spec`) perform their own claims.
...
- **Success path — one continuous lock.** `xez-open-pr` releases the **issue** lock as part of its handback, but only after its `--handoff xez-auto-review-pr` has transferred the chain lock onto the **PR**; the review and UI-QA steps re-enter that inherited lock (take-over comment, no release), and this skill releases the PR lock exactly once, at the end of the run (step 12) — or in step 9's cannot-run path. See the chained hand-off section above.
