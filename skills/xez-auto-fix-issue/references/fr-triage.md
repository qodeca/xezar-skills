...
The gate `xez-auto-fix-issue`'s **feature route** runs (step F1) before
...
with `NO_ACTION_NEEDED` and state that it belongs on `xez-auto-fix-issue`'s bug
...
`` 🤖 `xez-auto-fix-issue` — not ready ``, the same shape
`xez-auto-manage-issues` uses: the missing items as a list, addressed to the
...
bug-vs-feature classification and the xez-auto-fix-issue hand-off, as evidence>
```
