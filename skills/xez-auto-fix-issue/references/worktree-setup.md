...
Detailed procedure for step 5 (create) and step 12 (cleanup) of `xez-auto-fix-issue`. Never run in the user's primary worktree.
...
WORKTREE_PARENT="$REPO_ROOT/.local/tmp/xez-auto-fix-issue"
CREATED_WORKTREE=0
...
- This procedure applies to the bug route only; on the feature route the delegated skills (`xez-auto-write-spec`, `xez-auto-implement-spec`) own their own worktrees.
