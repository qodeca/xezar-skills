...
- The chain skills this skill invokes (`xez-verify-in-repo`, `xez-root-cause`, `xez-fix`, `xez-open-pr`, `xez-auto-review-pr`; feature route: `xez-auto-write-spec`, `xez-auto-implement-spec`) load the rest of the config themselves.
- A missing required chain skill stops the run and names the skill to install (`xez-open-pr` alone has an inline fallback — see `references/pr-finalize.md`).
