...
def markers: [ match("(^|\n)[ \t#*]*🤖[^\n]*"; "g") | .string ];
def marker_skills: [ markers[] | match("xez-[a-z0-9-]+"; "g") | .string ] | unique;
...
def opener_runs($cs):
  [ $cs[] | .body | opener_lines[] | [ match("xez-[a-z0-9-]+"; "g").string ] | unique[] ]
...
def classify($gap):
  ...
  | [ (if (($runs["xez-auto-review-pr"] // 0) >= 2) then "second review round" else empty end),
      (if ((($runs["xez-auto-continue-pr"] // 0) + ($runs["xez-auto-continue-pr-loop"] // 0)) > 0) then "resumed" else empty end),
      (if (($runs["xez-auto-fix-pr"] // 0) > 0) then "driven to merge-ready" else empty end),
      (if (($runs["xez-auto-qa-pr"] // 0) >= 2) then "second QA round" else empty end),
      (if ($state == "CLOSED") then "closed unmerged" else empty end) ] as $signals
  | [ (if ((($runs["xez-auto-create-pr-loop"] // 0) + ($runs["xez-auto-continue-pr-loop"] // 0)) > 0) then "resumed" else empty end) ] as $loop_only
