...
```markdown
⚠️ `xez-close-fixed-issues` closed 0 issues; two PRs mention open issues without recognized closing links.
...
fixed, add `"closeKeywords": ["zamyka", "naprawia"]` to `.xezar/pipeline/config.json`
