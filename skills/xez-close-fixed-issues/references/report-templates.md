...
plus the `xez-close-fixed-issues` attribution, so reruns do not duplicate them.

## Close comment (step 4a — merged into the base branch)

```markdown
🤖 `xez-close-fixed-issues` — issue closed
...
```markdown
🤖 `xez-close-fixed-issues` — non-base merge
...
```markdown
🤖 `xez-close-fixed-issues` — closed without merge
...
```markdown
✅ `xez-close-fixed-issues` processed {since} → {today}: closed {N}, commented {M}, skipped {K}, unmatched-mentions {U}, dry-run-would-have {X}.
...
single-word keyword to `.xezar/pipeline/config.json` and rerun.
```
