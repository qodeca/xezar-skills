...
10. **Run `xez-auto-review-pr` and apply fixes.** Run the PR's single authoritative code-review pass with `xez-auto-review-pr {prNumber} --autofix` (this run owns the PR) before the final summary comment, last pushes, or report. Follow its workflow verbatim: fixes land as new commits in the same worktree (never history rewrites); re-run targeted validation (the full step-8 gate when a fix reaches beyond a single module/test file); update the plan's Progress; loop until a clean verdict or only documented non-actionable findings remain. It claims and releases its own local ownership lock — do not second-guess that. If it cannot run, leave `Status: in-progress`, stop, and report the blocker. Full procedure and verdict handling: `references/review-report.md`.
