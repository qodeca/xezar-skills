...
How `xez-auto-create-pr` decides whether to run plain or hand the run to `xez-auto-create-pr-loop`. Orchestrating skills (`xez-auto-implement-spec`, `xez-auto-fix-issue`'s feature route) delegate to this rule by invoking `xez-auto-create-pr`, forwarding `--loop` verbatim only when the user passed it — they never add the flag on their own.
...
Hand off to `xez-auto-create-pr-loop` only when at least one holds:
...
Nothing else selects the loop: not UI work (plain runs post screenshots via `xez-auto-qa-pr` / **attach-image-evidence**), not "might not finish in one pass" (`xez-auto-continue-pr` resumes a plain run fine), not subjective "feels large".
...
The engine is decided **before** any plan is committed, because the two engines consume different plan formats: plain → the standard Progress-tracked execution plan (per `xez-auto-create-pr` step 4); loop → the run-folder in `xez-auto-create-pr-loop`'s format (`PLAN.md` Tasks table + `HANDOFF.md`/`NOTIFY.md`). A run's format is therefore fixed at creation — resume plain runs with `xez-auto-continue-pr`, run-folder runs (a `Tracking run folder:` line in the PR body, or a run-folder tracking path) with `xez-auto-continue-pr-loop`. Never re-apply the threshold to a run that already has artifacts.
