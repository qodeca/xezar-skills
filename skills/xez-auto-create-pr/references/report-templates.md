...
```markdown
{✅ or 🔁 or ⛔} `xez-auto-create-pr`: {what changed; ready, incomplete or blocked, and why.}
...
{Next review/QA decision, or first remaining Step and `xez-auto-continue-pr {prNumber}`.}
```

For a spec-only handoff, name `xez-auto-implement-spec {SPEC_PATH}`. Keep an
...
On a fresh `xez-auto-create-pr` run, include the routing line from
