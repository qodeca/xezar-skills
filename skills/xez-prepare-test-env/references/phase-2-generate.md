...
Full procedure for the generation phase of `xez-prepare-test-env`, steps 2.1–2.6,
...
interpolating it into `.xezar/pipeline/browsers/<provider>.md`; reject any other value.

1. If the provider was explicit and `$BROWSER_FILE` is missing, run
   `xez-setup-agent-pipeline` to install the selected descriptor, reload, and
...
that runs on **this** platform — `sh .xezar/pipeline/scripts/test-env-up.sh` on
macOS/Linux/WSL2/Git Bash, `pwsh -File .xezar/pipeline/scripts/test-env-up.ps1` on native
...
1. Record **why** in the repo-local override file (`.xezar/pipeline/overrides/xez-prepare-test-env.md`),
...
2. Fall back to the agent-driven flow for the unscripted part on each run —
   ...
   normally.
3. Re-attempt generation when the blocker changes (the override-file note is
