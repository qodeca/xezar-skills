...
| --- | --- | --- |
| Entrypoint (up) | `.xezar/pipeline/scripts/test-env-up.sh` (`test-env-up.ps1` on native Windows) | The one command that brings the env up fast |
| Teardown (down) | `.xezar/pipeline/scripts/test-env-down.sh` (`test-env-down.ps1` on native Windows) | Stops exactly what the up script started |
| Environment descriptor | `.local/qa/test-env.json` | What consumers (QA, integration tests) attach to |
| Build cache state | `.local/qa/test-env-build-cache.json` | Written/read by the up script, not by the agent |
...
- **POSIX `sh`** (`.sh`) on macOS, Linux, WSL2, and Git Bash/MSYS on Windows.
  Run with `sh .xezar/pipeline/scripts/test-env-up.sh`.
- **PowerShell** (`.ps1`) on native Windows (the user works in PowerShell or
  cmd, with no WSL/Git Bash available). Run with
  `pwsh -File .xezar/pipeline/scripts/test-env-up.ps1` (or `powershell -ExecutionPolicy
...
- `--browser-provider <name>` (optional) — override `browser.provider` for this
  ...
  the matching `.xezar/pipeline/browsers/<name>.md` must exist.
...
**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-prepare-test-env.md` when present; safety rules still win.

0. **Agentic setup** — follow `references/agentic-setup.md`: load
   `.xezar/pipeline/config.json` via the standard snippets (missing config → the
...
1. **Execute the saved entrypoint (every run — "Phase 1" in the references).**
   ...
   if [ -f "$UP_SCRIPT" ] && grep -q 'xez-prepare-test-env: generated entrypoint' "$UP_SCRIPT" \
      && [ "$REGENERATE" != 1 ]; then
   ...
   if ((Test-Path $UpScript) -and
       (Select-String -Quiet 'xez-prepare-test-env: generated entrypoint' $UpScript) -and
       -not $Regenerate) {
   ...
   - **Script succeeds** → read `baseUrl` from `$ENV_DESCRIPTOR`, print the
     ...
     run so consumers (`xez-auto-qa-pr`, `xez-integration-tests`) attach to the
   ...
   The marker line (`# xez-prepare-test-env: generated entrypoint`) is how the
...
2. **Generate the entrypoint (first run, `--regenerate`, or repair — "Phase 2"
   ...
   - **2.4 Ensure the configured browser provider** — once, through its
     descriptor `.xezar/pipeline/browsers/<provider>.md`.
...
3. **Bake every lesson back into the scripts (self-improvement).** **Any
   ...
   3. Append the exact working command chain (and the failure it prevents) to
      the repo-local override file at `.xezar/pipeline/overrides/xez-prepare-test-env.md` —
...
- Ensure the configured browser provider at generation time through
  `.xezar/pipeline/browsers/<provider>.md`; when installation or its live-launch check fails,
