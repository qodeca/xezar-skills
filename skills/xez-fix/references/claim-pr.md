...
- The chain driver (`xez-auto-fix-issue`, via `xez-verify-in-repo`) already ran the three-signal in-progress check before this step started; xez-fix does not re-litigate ownership — it applies the idempotent claim on `{issueId}`.
...
- **Do not release the lock here.** The release happens in `xez-open-pr` (success path) or via an external janitor (failure path); on `Status: blocked` the lock intentionally remains set so a human can pick it up.
