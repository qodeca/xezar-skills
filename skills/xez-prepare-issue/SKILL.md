...
3. **Author a spec and land it on a PR** (feature needs a spec, none exists) — follow `references/spec-when-missing.md`: create the tracking issue first (step 5, so there is a number to link), then delegate to **`xez-auto-write-spec {issueId}`**, which writes the spec autonomously, opens a **ready spec PR** with `Refs #{issueId}`, and emits the `Spec:` and `PR:` reference lines. Comment the spec path and PR link back onto the issue via **comment-issue**. Implementation happens later via `xez-auto-implement-spec {SPEC_PATH}` or `xez-auto-fix-issue {issueId}` (both keep the spec PR design-only and ship the implementation on its own PR referencing it). This is the one path on which `xez-prepare-issue` produces a PR — it is a **design** (a spec), never implementation.
...
- Tracker-only by default: never edit, commit, or push repository files. The one exception is step 3 — a feature that needs a spec and has none — where this skill produces a **spec PR** (a design document only, never implementation) by delegating to `xez-auto-write-spec`, then links it on the issue.
