...
The step 3 branch of `xez-prepare-issue`: when the duplicate/spec search (steps 1–2)
...
which `xez-prepare-issue` creates a PR, and the PR contains a **spec document only**
...
2. **Delegate spec authoring to `xez-auto-write-spec {issueId}`** — the dedicated
   ...
   writes the spec via `xez-spec-writing --autonomous` **including its Open
...
commit. Implementation happens later via `xez-auto-implement-spec {SPEC_PATH}` or
`xez-auto-fix-issue {issueId}` — both keep the spec PR design-only and ship the
implementation on its own PR referencing it (`Refs #{specPr}`). Report both the
issue and the spec PR in step 6.

## Guardrails

- The PR is design-only. Delegate to `xez-auto-write-spec` (which never
  implements) — never to `xez-auto-fix-issue` / `xez-auto-implement-spec` on
