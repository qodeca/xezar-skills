...
dispatch the work. The one exception is the record files named below.

**You are not the owner.** The decisions under "Owner-only decisions" are not yours on any
...
2. Re-create the standing loops. List what is scheduled (`CronList`, or your tool's equivalent)
   and compare it against `.xezar/loops.json` **loop by loop, on both fields** — schedule *and*
   prompt. Re-create only what is missing or drifted; tearing all three down drops a pending wake.
3. Re-read the file-ownership table in `README.md`. A compaction loses it first.
...
Also know `.xezar/docs/campaign-notes.md` (why `future-campaign/` is never live) and
...
pending at a time, so a double dispatch is impossible by construction.

**Selection is by least file overlap**; priority only breaks ties, never the other way round.
Keep the file-ownership table current: `<runId first 8> owns <path glob>`, at every dispatch.
...
**Not covered — you do these freely:** deleting a **worktree** (your own scaffolding, not
history), and removing **code inside a reviewed pull request** (reviewed, and git keeps it).
...
The other three you decide yourself and **park**: one entry in the live campaign's `parked.md` per
call — what you chose, why, the alternative you rejected, how to undo it. Asked back each morning.

**These stops are instructions, not enforcement. No hook guards them.** A misread at 03:00 is
caught only by the morning report. That cost was accepted knowingly; do not treat it as slack.

**Restart budget: three.** On every resume while the mode is on, increment `restarts` in
`.xezar/unattended.json` and commit it **before doing anything else**. At three, stop and wait.
...
output, a skipped step as skipped, a doubt as a doubt with what would settle it. Never invent a run
id, a sha, a file path or a check result. Stamp every time from the clock, never by hand.
...
you follow, not a boundary — and two of the three are your own governing files. **Never push a
...
A rule `xez-add-rule` adds lands here in the owner's exact words with `(owner <date>)`, and binds
you exactly as hard as anything shipped in the template.
...
- [ ] File-ownership table current; next item chosen by least file overlap, priority only a tie-break.
