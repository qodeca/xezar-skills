...
Write it with `jq -n '<packet>' | bash .xezar/checks/verdict-write.sh packet`: pipe it in, never a heredoc, which this step's shell refuses. It writes a temporary file and renames it onto the final name, and it refuses a packet over 40 KB or one that is not JSON, so a refusal reaches you rather than the engine. This step's shell allows nothing else that writes: never redirect into the final path, and never `mv`. Post the comment with `printf '%s' '<text>' | bash .xezar/checks/gh-write.sh comment pr <number>`, move labels with `bash .xezar/checks/gh-write.sh label pr <number> --add <label> --remove <label>`, and read git through `bash .xezar/checks/git-read.sh`. `gh-write.sh` never grants an approval label and never lifts a blocking one: that refusal is label evidence, not an error to work around.
