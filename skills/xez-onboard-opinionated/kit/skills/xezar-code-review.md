...
Write it with `bash .xezar/checks/verdict-write.sh packet`, the packet on stdin. It writes a temporary file and renames it onto the final name, and it refuses a packet over 40 KB or one that is not JSON, so a refusal reaches you rather than the engine. This step's shell allows nothing else that writes: never redirect into the final path, and never `mv`. Post the comment with `gh pr comment <number> --body-file -`, the text on stdin, and read git through `bash .xezar/checks/git-read.sh`.
