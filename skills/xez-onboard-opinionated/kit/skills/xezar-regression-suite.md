---
name: xezar-regression-suite
description: Curate the suite of tests that pins every past bug so it cannot come back
---

# Curate the regression suite

Every bug this project has fixed is a thing the product once got wrong in front of somebody. The regression suite is the list of those, as tests, so that none of them happens twice. `xezar-bug-investigation` adds the red test for the bug it is fixing; you look after the suite as a whole — what is in it, whether each test still guards what it claims, and what is missing.

## What belongs in it

A test is in the regression suite when it pins a fault that **really happened**. Each one carries, in the test itself:

- **the issue it guards**, by number, so a failure names its own history;
- **what went wrong, in one sentence a newcomer can read**;
- proof it is a real guard: it was **watched failing on the code without the fix**, in this run or in the run that added it, and the record says which.

A test that passes both with and without the fix guards nothing. Find those — they are the quiet rot in every old suite — and either make them fail for the right reason or report them.

## The work

1. **Find the gaps.** Read the closed bug issues and the fix commits. A fix that landed with no test that fails without it is a gap; list it, and close the ones in scope by writing the missing test against the behaviour the issue describes.
2. **Keep it honest.** A regression test asserts the behaviour a person saw break, at the level they saw it. Do not rewrite one into an implementation detail because the code moved.
3. **Keep it runnable.** The suite runs with the installed pinned runner and is selectable on its own — a tag, a folder, a naming rule; use what the project has. Slow or flaky members are fixed or reported by name; they are never silently skipped.
4. **Never lower a threshold**, never loosen an assertion to get green, never delete a test because it is inconvenient.

## Retiring a test

A suite that only grows stops being read. A test leaves the suite for exactly one reason: the behaviour it guards was **removed on purpose**. Retiring it needs the owner's word — quote it, with the issue or PR where it was given — and the reason, recorded where the test was. A feature that still exists never loses its regression test, however old the bug.

Inputs: the area to curate, or the bugs to cover. Output: the tests added, the false guards found, the gaps that remain and why, the tests retired with the authority for each, and how to run the suite alone. Run the focused tests for what you changed; the workflow's gates run the rest.
