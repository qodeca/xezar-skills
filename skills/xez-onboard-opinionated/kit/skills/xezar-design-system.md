---
name: xezar-design-system
description: Create, maintain and update the design system every design is judged against
---
...
The design and review roles open these by name, so the names are a contract. Keep every one of them, as a `.md` file directly under `<system>`:

| page | what it holds |
|---|---|
| `README.md` | the entry point: what the system is, the task route (which page to open for which job) and the review route, in order |
| `known-gaps.md` | what the system does not cover yet, stated plainly. The first page a reviewer opens; an honest gap here is worth more than a rule nobody follows |
| `usage.md` | how to scope a design brief: what counts as a new surface, a change, a fix-sized edit |
| `recipes.md` | compositions that recur — task tables, threads, settings, overlays, states — and the components each is built from |
| `patterns.md` | interaction patterns and when each applies |
| `components.md` | every component: its purpose, its states, its tokens, its accessibility contract |
| `behaviour.md` | motion, focus, keyboard, announcements, responsive behaviour |
| `verification.md` | what evidence a design needs and how it is captured: themes, widths, states |
| `lifecycle.md` | who owns a design, the statuses Draft → In review → Approved, what moves a design between them, what exact-content acceptance means |
| `storage.md` | where mockups, handoffs and captures live, and what may never become a dependency of a maintained document |
| `new-designs.md` | the template for a feature folder. §4 lists the states every screen is shown in — default, empty, loading, error, refusal, phone; §5 lists appearance — theme, accent, density, width |
| `writing.md` | the voice of interface copy: labels, errors, empty states |

Beside them sits the **stylesheet** every mockup links. Tokens first — colour, type, space, radius, motion — then components built only from tokens. A value that appears in a component and not as a token is a gap; add the token or record the gap.

You also own **the heading list of the designs index**: `README.md` in the folder `paths.designs` names lists the headings every feature README must carry. Change that list here and nowhere else, and say in your handoff which existing feature folders it makes stale.

## How the system changes

**Extract before you invent.** A new rule comes from something the product already does in more than one place, read in this run and cited. When two screens disagree, that is an open decision for the owner, recorded in `known-gaps.md` — not a quiet pick.

**A change to the system is a change to every design that uses it.** Before renaming or removing a token, a component or a page, find what uses it — the mockups under the designs folder, the application's own styles — and either move them in the same change or list them in the handoff as the follow-up the owner must schedule. A rename that leaves the old name working nowhere is a breaking change and the PR says so.

**Accessibility is part of a component, not a review of it.** Every component states its keyboard behaviour, its focus treatment, its name and role, and its contrast in both themes. A component without that is a draft and is marked as one.
