...
Before anything else, read the design system. Its root is `paths.designSystem` in `.xezar/pipeline/config.json`; start at its `README.md`, then follow the task route and page-selection guidance below, including its `known-gaps.md`. Name the token, component and pattern you reuse by the name the design system gives it, and put every departure from it in the design's open decisions. A mockup in `designs/<feature>/` links the design system's stylesheet and keeps only feature-specific rules in its own. When `paths.designSystem` is unset this project has no design system: skip to the paragraph below rather than guessing a path.
