...
Detailed procedure for step 3 (create) and step 10 (cleanup) of `xez-auto-continue-pr`. Never resume in the user's primary worktree.
...
## xez-auto-continue-pr specifics — create the worktree from the PR head (step 3)

Unlike `xez-auto-create-pr` (which branches a fresh `$BRANCH` off `origin/$BASE_BRANCH`), this skill checks out the **existing PR head**. `HEAD_REF` and `IS_CROSS` are filled via **get-pr** (fields `headRefName`, `isCrossRepository` — already part of the step 1 fetch). On the cross-repository path, use the **checkout-pr** operation to make the PR head available locally.

```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
GIT_DIR=$(git rev-parse --git-dir)
GIT_COMMON_DIR=$(git rev-parse --git-common-dir)
WORKTREE_PARENT="$REPO_ROOT/.local/tmp/xez-auto-continue-pr"
CREATED_WORKTREE=0
