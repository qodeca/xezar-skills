...
Detailed procedure for step 7, the single automated review loop of `xez-auto-continue-pr`.

## Automated review with `xez-auto-review-pr` (step 7)

Before you post the final summary comment, push the final changes, or flip the PR body to `complete`, subject the resumed PR to its authoritative review with the `xez-auto-review-pr` skill. Do not run a separate direct `xez-code-review` pass first: `xez-auto-review-pr` invokes that engine verbatim and applies compatibility, security, API-contract, breaking-change, and scope checks.
...
4. Loop, **at most twice**, until `xez-auto-review-pr` returns a clean verdict (no actionable blockers) or the remaining findings are non-actionable (out-of-scope, false positive) and explicitly documented in the summary comment you post in step 8.
...
- **Cannot run** (e.g., required checks not yet green, missing context) → stop here, leave `Status: in-progress` in the PR body, document the blocker in the summary comment, and tell the user how to re-enter (`/xez-auto-continue-pr {prNumber}`).

## xez-auto-continue-pr specifics

- Lock re-entry: `xez-auto-review-pr`'s claim check recognizes that the current user already owns the `in-progress` lock from step 1, so it proceeds as re-entry without re-claiming. This skill keeps owning the lock and releases it in step 9.
