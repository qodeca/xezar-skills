...
branch (`$BASE_BRANCH`), using the document as the brief for `xez-auto-create-pr`.
...
Use the stable marker `` 🤖 `xez-followup-issue-from-pr` — tracking implementation ``
...
```markdown
✅ `xez-followup-issue-from-pr` {created/reused} {linked issue title} to {remaining outcome}.
