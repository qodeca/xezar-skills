...
This skill never boots the app itself — `xez-prepare-test-env` provisions a runnable
...
- `--keep-env` (optional) — leave the environment running on exit even if this run
  ...
  `xez-prepare-test-env --stop`.
- `--artifacts <dir>` (optional) — override the artifacts directory. Default:
  `<paths.qa>/artifacts_<runId>` (default `.local/qa/artifacts_<runId>`).
- `--force` (optional, PR mode) — bypass the in-progress claim check to take over
  a PR another actor claimed.

## Chaining

In PR mode this skill consumes a `{prNumber}` (the `PR:` reference line a PR-producing skill emitted) and posts screenshot QA evidence back to that existing PR; it is read-only on source and never opens a PR. PR mode ends by reporting the `PR:` / `Issue:` chaining reference lines; in local mode the artifacts folder is the deliverable. Companion skills: `xez-auto-review-pr` (review-first gate), `xez-prepare-test-env` (boots/provisions the instance and browser), `xez-integration-tests` (follow-up automated UI test), `xez-setup-agent-pipeline` (installs a missing browser provider) — each runs verbatim; a missing required one stops the run naming the skill to install.

## Workflow

**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-auto-qa-pr.md` when present; safety rules still win.
...
1. **Resolve the mode.**
   - **PR mode** — `{prNumber}` was given AND `$TRACKER` is non-empty AND the
     descriptor file `.xezar/pipeline/trackers/${TRACKER}.md` exists. Read that descriptor;
...
3. **Review-first gate (PR mode only).** QA runs **after** code review. Check the
   ...
   - **Not reviewed** — no approve/changes-requested `reviewDecision` (null /
     ...
     invoke **`xez-auto-review-pr {prNumber}`** verbatim first (it re-enters the
...
5. **Detect whether the change already ships a UI test.** Look in the diff for
   ...
   (discover it the way `xez-integration-tests` does: an `__integration__/`,
...
7. **Boot the app via `xez-prepare-test-env`.** Never boot by hand. Invoke the
   `xez-prepare-test-env` skill (mode `auto`; `--no-ephemeral` when the app needs
...
12. **Follow-up UI-test scenario (only when `HAS_UI_TEST` from step 5 is
    ...
    `xez-integration-tests` — a second PR comment in PR mode (**comment-pr**), or
...
13. **Labels, teardown, and lock release.**
    ...
    **Teardown (run in a `trap`/finally):**
    - Tear down the environment only if this run started it and `--keep-env` was
      not set — invoke `xez-prepare-test-env --stop`. Otherwise leave it running
      for reuse.
    ...
    - **PR mode, inherited chain lock** (re-entry — a flow runner such as
      `xez-auto-fix-issue` or `xez-auto-fix-pr` handed the lock off to this run):
...
- Boot the app only through `xez-prepare-test-env`; reuse a running environment;
  tear down only an environment this run started (unless `--keep-env`).
- Drive the UI only through the selected `.xezar/pipeline/browsers/<provider>.md` operations;
