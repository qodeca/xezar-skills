...
The machine- and human-readable report artifacts `xez-auto-qa-pr` writes
...
Automate [this verified scenario]({evidence link}) with `xez-integration-tests`:
...
```markdown
📸 `xez-auto-qa-pr`: {PASS | FAIL | PARTIAL} — {behavior verified and result or blocker}.
