# Boot the app via xez-prepare-test-env

Detailed procedure for step 7 of `xez-auto-qa-pr`. Do not boot the app by hand. Invoke the `xez-prepare-test-env` skill (mode `auto`; pass `--no-ephemeral` when the app clearly needs no backing services). It discovers or provisions a runnable instance, installs the configured browser provider when missing, and writes the environment descriptor. Then read the descriptor:

```bash
QA_DIR=$(jq -r '.paths.qa // ".local/qa"' .xezar/pipeline/config.json 2>/dev/null || echo ".local/qa")
...
setup error — invoke `xez-setup-agent-pipeline` to install it, then retry.
...
same way, without quoting them back. If `xez-prepare-test-env` reports the app
