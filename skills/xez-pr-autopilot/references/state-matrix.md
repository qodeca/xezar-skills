...
| 1 | A plan exists with pending steps (`- [ ]`, or a Tasks row not `done`) | `xez-auto-continue-pr {pr}` — or `xez-auto-continue-pr-loop {pr}` when the tracking line names a **run folder** | all plan steps done |
...
| 3 | Spec-only diff (only `$SPECS_DIR`, the config's `paths.specs`) | `xez-auto-review-pr {pr}` (specification review). **Never** grow it into implementation — that ships on its own PR via `xez-auto-implement-spec` | spec review submitted |
| 4 | Mergeability is conflicting or behind the base (**always run before rows 5 and 6** — a conflicted branch makes review and CI judge a diff that will never merge) | `DRIVABLE`: `xez-auto-fix-pr {pr}` (it merges the base **first**, before review or CI work). Not `DRIVABLE` (another author's PR): `xez-auto-review-pr {pr}` only — it owns the base handling for a PR that is not yours, and no autofix commit lands on the author's branch | mergeable, or the base state reported to the author |
| 5 | Review is none / required, or changes-requested, or unresolved conversations remain | `DRIVABLE`: `xez-auto-fix-pr {pr} --max-iterations <n>` (review + autofix + CI + UI in one loop). Not `DRIVABLE` (another author's PR): `xez-auto-review-pr {pr}` only — review and hand off, no autofix | approvable, no unresolved blocking conversations |
| 6 | CI red, everything else already fine | `xez-auto-fix-pr {pr} --ci-only` | all required checks green, or the bounded wait expired and the run reported `Unverified` |
| 7 | The diff is UI-touching and no QA evidence exists (QA required, no approval and no evidence) | `xez-auto-qa-pr {pr}` — capture screenshots and a pass/fail report | evidence attached, or a documented reason UI QA cannot run |
| 8 | Review findings intentionally not fixed (nits, low severity, out of scope) | `xez-followup-issue-from-pr` per finding, idempotently | each finding tracked |
| 9 | Approvable + green + QA satisfied | **default: stop at merge-ready** and report. With `--allow-merge`: `xez-approve-merge-pr {pr}` | reported / merged |
...
- **`PUSHABLE` governs the mechanism, `DRIVABLE` governs the permission.** They
  ...
  commits on a head branch (`xez-auto-fix-pr`, and `xez-auto-review-pr --autofix`)
...
- **Fork PRs — split on `PUSHABLE`, not on `IS_FORK`.** Whether the head branch
  ...
  - Not `PUSHABLE` (someone else's fork): you cannot push to the contributor's
    branch. Do not force a base merge — `xez-auto-fix-pr` hands that to
    `xez-auto-review-pr`'s fork carry-forward flow, which opens a credited
...
- **Draft PR:** diagnose and fix normally; promotion to ready happens inside
  `xez-auto-fix-pr`'s merge-prep step, not here. A spec-only design PR and any PR
...
- **`xez-auto-fix-pr` already contains** review + CI + UI QA + follow-ups. When
