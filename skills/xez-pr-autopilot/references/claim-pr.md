...
## xez-pr-autopilot specifics

- **Outer lock.** This skill claims `{prNumber}` once (assignee via **assign-pr** + `in-progress` via the `apply_label` guard + 🤖 claim comment) and holds the **outer** claim for the whole run. Every sub-skill it dispatches will see `$CURRENT_USER` already owns the PR and treat its own claim as re-entry — that is expected, do not fight it, and do not let a sub-skill's release drop the outer lock: this skill owns it until the final report.
- **Fetch fields.** Read the PR via **get-pr** with fields `number,title,state,author,assignees,labels,isDraft,headRefName,headRepositoryOwner,isCrossRepository,baseRefName,url`. Stop immediately when the PR is already merged or closed.
- **Release on every exit.** Register a `trap`/finally at claim time that releases the outer lock (label removal via the guard, **unassign-pr** where the convention expects it, plus the completion comment) on any exit path — success, blocker, or crash.
- **Degraded claim (no triage rights).** An account without triage permissions cannot assign or label; **assign-pr** and `apply_label` then fail with a permission error. Do not retry or work around it: the claim degrades to the 🤖 claim comment alone — still a valid signal, since the three-signal check accepts any one of them — the release path tolerates the absent `in-progress` label, and the degraded claim is stated in the run report and the summary comment. Same rule as the label-permission fallback in `references/report-templates.md`.
- **`--dry-run` takes no claim.** A dry run mutates nothing: skip the claim, the comments, and the labels entirely, and say so in the report.
