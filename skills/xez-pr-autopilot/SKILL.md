---
name: xez-pr-autopilot
description: Diagnose what state one open PR is actually in — unfinished plan steps, missing review, unresolved conversations, red CI, base conflicts, missing QA evidence, merge-ready — then run the matching chain of xez-* skills in order and publish a status report. Use for "finish PR 123", "what is left on PR 123", "drive PR 123 to the end".
---
...
to the existing `xez-*` skills and never re-implements their logic.

It is the dispatcher that sits above `xez-auto-continue-pr` (finish the
implementation), `xez-auto-fix-pr` (drive to merge-ready), `xez-auto-qa-pr` (UI
evidence), and `xez-approve-merge-pr` (merge). Use those directly when you
...
- `--allow-merge` (optional) — permit the chain to end in an actual merge via
  `xez-approve-merge-pr`. **Off by default**: the run stops at merge-ready.
- `--force` (optional) — take over an `in-progress` claim held by another actor.
- `--max-iterations <n>` (optional) — forwarded to `xez-auto-fix-pr`. Default `3`.
...
`xez-auto-review-pr` flow rather than here. It ends by reporting the `PR:` and
`Issue:` chaining reference lines. Companion skills, each invoked **verbatim**:
`xez-auto-continue-pr`, `xez-auto-continue-pr-loop`, `xez-auto-fix-pr`,
`xez-auto-review-pr`, `xez-auto-qa-pr`, `xez-followup-issue-from-pr`,
`xez-approve-merge-pr`. A missing companion stops the run and names the skill to
install — never improvise a replacement for it. Only a skill this run can
actually dispatch belongs on that list: `xez-merge-buddy` scans the whole open
queue read-only rather than driving one PR, so it is deliberately absent and its
absence never stops a run.

## Workflow

**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-pr-autopilot.md` when present; safety rules still win.

0. **Agentic setup** — follow `references/agentic-setup.md`: load
   `.xezar/pipeline/config.json` plus the tracker descriptor (auto-run
   `xez-setup-agent-pipeline` when missing), apply the repo-local override
...
- **Never merge implicitly.** The chain stops at merge-ready unless
  ...
  `xez-approve-merge-pr` owns the merge.
...
- **Spec-only design PRs stay design-only.** Implementation ships on its own PR
  via `xez-auto-implement-spec`; never grow a design PR into implementation here.
