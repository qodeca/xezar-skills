...
3. **Enforce label blocks and the QA gate.**
   ...
   - **Decide with the script, not by narrating.** Collect the facts from steps 1-3 into one JSON object and pipe it to `references/merge-gate.sh`. It prints one `NAME=value` line per gate plus a `Gate:` verdict, and its **exit code is the decision**: `0` merge allowed, `1` refused, `3` cannot decide. Quote its output in the report. The script is authoritative over your own reading of the same facts — that is the point of it, because "unknown is never a pass" means nothing when a model can simply write "pass". If you cannot execute it, apply the rules in its header comment, which state the same algorithm.
