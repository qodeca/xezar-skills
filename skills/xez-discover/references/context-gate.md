...
What `xez-discover` checks before it writes anything, and what it hands back when material is missing. The gate exists because before Intake there is no repository to verify claims against: without it the agent fills the brief with plausible fiction.
...
The ticket-level items of the Definition of Ready (`SDLC.md`) — the problem and who has it, the expected outcome and its check, what is out of scope — must reach *has material* with a source that is not `[SYNTHETIC]` or `[ASSUMPTION]`; every blocking question must be answered by the person who can answer it; and where spec PRs exist, their autonomous assumptions must be confirmed. Only then can `xez-prepare-issue` start filing from the brief. Say this in the report when it is not yet the case.
