...
How `xez-discover` differs by situation (workflow step 1, and the ladders in step 3). The brief's structure is one; the mode changes where discovery starts, which sections are mandatory beyond the common ones, who signs the Definition of Ready, and what the skeptic attacks first.
