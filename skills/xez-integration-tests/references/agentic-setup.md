...
## xez-integration-tests specifics

- This skill performs **no tracker operations and no label mutations**, and the pipeline config is **optional**: when `.xezar/pipeline/config.json` is absent, skip Preflight steps 1–2 and continue with repo discovery — do not force `xez-setup-agent-pipeline`. When the config is present, consume `validation.commands` and `paths` (notably `paths.qa`, default `.local/qa`, where the shared test-env descriptor `test-env.json` lives).
- Agent-driven UI exploration goes through the configured browser-provider descriptor `.xezar/pipeline/browsers/<provider>.md` and its named operations (**open**, **snapshot**, **interact**, **assert**, **screenshot**, **close**); only the implicit legacy Playwright provider may use embedded fallback instructions when an older repo has no descriptor.
