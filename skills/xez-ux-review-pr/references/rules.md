...
- **Interactive run — a user is in the loop.** This skill acts once, may ask
  questions, reports, and hands control back. It is not an `xez-auto-*` skill:
...
- **Marker contract.** The review comment carries the marker
  `` 🤖 `xez-ux-review-pr` — evidence-first design review `` on its own first
...
- Scope: this skill judges the increment a PR ships. Whole modules, flows, or
  existing product areas belong to the `xez-ux-shape` skill in Review mode,
