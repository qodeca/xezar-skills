...
- **Never auto-force in batch mode.** Stale-lock recovery and the `--force` override above do not apply to the sweep — leave a claimed PR alone; a human can re-run a single PR via `xez-auto-review-pr` with `--force` if a takeover is really wanted.
- When `labels.enabled` is `false`, the label signal is unavailable: treat a foreign assignee as the claim signal.
- Per-PR claiming, lock refresh, and release are owned entirely by the delegated `xez-auto-review-pr` run — do not second-guess or duplicate them.
