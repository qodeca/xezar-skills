...
The credited-author resolution algorithm `xez-auto-update-changelog` runs in
...
`xez-auto-review-pr` writes this template when it carries a fork PR forward. Regex (anchored to the first 20 lines of the body, case-insensitive):
...
Same template, also written by `xez-auto-review-pr`. Regex:
...
When neither body regex on the merged PR matches, the carry-forward flow still leaves an authoritative trail on the **original** PR via `xez-auto-review-pr`'s closing-comment template:
...
Contributors get handed off in prose that matches none of the `xez-auto-review-pr` templates. Scan every merged PR body (case-insensitive) for:
