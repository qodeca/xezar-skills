---
name: xez-auto-update-changelog
description: Draft a CHANGELOG.md release entry in an emoji-driven format for every PR merged since the last release, then delegate to xez-auto-create-pr so it lands as a docs PR against the configured base branch. Honors the Supersede Credit Rule and verifies every credit against commit authorship, so carry-forwards and umbrella merges credit the contributor rather than the merger. Use at release time.
---

# Auto Update Changelog

Release-engineering skill. Compile a `CHANGELOG.md` entry for the unreleased window, then hand the file edit off to `xez-auto-create-pr` so it lands as a normal docs PR against the configured base branch.
...
- `--dry-run` (optional) — print the drafted entry to stdout; do **not** edit `CHANGELOG.md` and do **not** invoke `xez-auto-create-pr`.
- `--slug <kebab-case>` (optional) — override the slug `xez-auto-create-pr` uses. Default: `changelog-<version>`.

## Chaining

This skill drafts a `CHANGELOG.md` entry and delegates the PR mechanics to `xez-auto-create-pr` — branch, worktree, commit, docs-only gate, labels, the `xez-auto-review-pr` autofix pass, and the summary comment. `xez-auto-create-pr` opens the PR (checking for an existing changelog PR first) and emits the `PR:` chaining reference line; this skill surfaces that PR URL in its own report. Companion skills: `xez-auto-create-pr` (required — the run stops if it is missing) and, optionally, `xez-close-fixed-issues`, which consumes the same window of merged PRs.

## Workflow

**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-auto-update-changelog.md` when present; safety rules still win.
...
- Never fabricate a Highlights paragraph. Leave the `<!-- TODO: Highlights -->` marker for the human author to fill in; `xez-auto-create-pr`'s review pass will call it out.
...
- Never run the full validation gate directly. Delegate to `xez-auto-create-pr` and let it decide.
- Never pass `--force` to `xez-auto-create-pr`. If a changelog PR for the same version already exists, stop and ask the user.
- Respect `--dry-run` absolutely: no file edits and no `xez-auto-create-pr` invocation.
...
- Runs well after `xez-close-fixed-issues` — the two skills consume the same window of merged PRs but mutate different surfaces (issue tracker vs `CHANGELOG.md`).
- The generated entry is intentionally a *draft*: a maintainer fills in Highlights and adjusts the narrative; `xez-auto-create-pr` opens the PR in `review` so they see it before merge.
