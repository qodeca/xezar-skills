...
`--plan <path>` requires the `Tracking plan:` / `Status:` fields and Progress
pointer. When reusing a PR, preserve any existing tracking fields and run-folder
metadata; refresh only agent-owned prose. A caller's spec-PR variant keeps its
`Source doc:` and `Status:` fields even without an execution plan.
