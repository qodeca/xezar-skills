...
## xez-open-pr specifics

This skill does not open a new claim of its own: it inherits the `in-progress` lock an earlier chain step claimed (typically `xez-fix`, driven by `xez-auto-fix-issue`) and owns the **Release / handback** phase — its step 8 — plus, with `--handoff`, the lock **transfer** below. Skip the issue phase when no `{issueId}` was given (brief- or spec-driven runs).
...
3. **comment-pr** — post on `{prNumber}`:
   ...
   ```
   🤖 `xez-open-pr` completed: PR opened. Lock handed off to `{next-skill}` — chain continues on this PR.
   ```
...
```
🤖 `xez-open-pr` — completed: opened ${PR_URL:-(no PR — aborted)}. Lock released.
```
