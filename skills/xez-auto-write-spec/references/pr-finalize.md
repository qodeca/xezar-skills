...
- Issue-driven run: invoke `xez-open-pr {issueId} documentation --title "docs(specs): ${TITLE}"` (add `--draft` when the high-stakes guard applies) and capture the PR number and URL from its `PR:` reference line.
...
```markdown
🤖 `xez-auto-write-spec` — 🏷️ label rationale
...
After the PR exists, publish the step-5 visuals via **attach-image-evidence**: `{prNumber}`, a short scenario report (a table mapping each image to its screen and current/proposed role), slug `spec-${SLUG}`, and the image paths — so they render inline on the PR, the same mechanism `xez-auto-qa-pr` uses. When the descriptor cannot render inline, it still posts the comment with links — surface the limitation, don't fail.
...
```markdown
## 🤖 `xez-auto-write-spec` — run summary
...
Implement with `xez-auto-implement-spec ${SPEC_PATH}`; use
`xez-auto-continue-pr {prNumber}` for further design work on this PR.
```
...
Chained consumers (`xez-auto-implement-spec`, `xez-auto-review-pr`, orchestration scripts) parse these exact text markers — never rename, translate, or decorate them.
