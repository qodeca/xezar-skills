...
## xez-auto-write-spec specifics

- Only **issue-driven** runs (`{issueId}` given) claim: apply the three-signal check to the FR issue via **get-issue**, claim with **assign-issue** + the `in-progress` guard + the `🤖` claim comment (**comment-issue**). **Brief-driven runs skip the claim entirely.**
- Before claiming, check via **search-prs** whether an open PR already references the issue with a spec — if so, STOP and point at it (never open a duplicate spec PR).
- Release/handback happens in step 8. Delegate to `xez-open-pr` when it ran — its hand-off step already performs the handback and lock release. Inline otherwise:
  ...
  3. Remove the `in-progress` label via the guard, then post the release comment: `` 🤖 `xez-auto-write-spec` — completed: opened {PR_URL}. Lock released. `` (substitute `(no PR — aborted)` when no PR opened).
