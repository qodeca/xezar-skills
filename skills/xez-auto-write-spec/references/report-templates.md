...
```markdown
📝 `xez-auto-write-spec`: {what the proposal enables and whether its PR is ready or draft.}
...
Continue with `xez-auto-implement-spec {SPEC_PATH}` for implementation on its own PR.
```
