...
Detailed procedure for step 2 (create) and step 8 (cleanup) of `xez-auto-write-spec`. Never run in the user's primary worktree.
...
WORKTREE_PARENT="$REPO_ROOT/.local/tmp/xez-auto-write-spec"
CREATED_WORKTREE=0
BRANCH="spec/${SLUG}"
...
- A spec run is docs-only: the dependency-install step is usually unnecessary. Install only when step 5's mockup/screenshot capture needs the app booted, and even then prefer the shared test environment the `xez-prepare-test-env` descriptor provides over provisioning inside this worktree.
