...
- `xez-prepare-test-env` has produced (or can produce) the shared test-env descriptor, and `.xezar/pipeline/config.json` names a browser provider whose descriptor exists at `.xezar/pipeline/browsers/<provider>.md`.
...
1. Boot the app through the test-env descriptor (same flow as `xez-auto-qa-pr` step 4 — reuse a healthy running env when the descriptor says it is fresh).
