...
Post only when `xez-spec-writing --autonomous` resolved Open Questions. Use
...
```markdown
🤖 `xez-auto-write-spec` — Open Questions answered with autonomous defaults
...
`xez-auto-write-spec` or `xez-auto-implement-spec {SPEC_PATH}`.
```
