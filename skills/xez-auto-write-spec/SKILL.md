---
name: xez-auto-write-spec
description: Autonomously turn a brief or FR issue into a spec landed on a ready PR — runs xez-spec-writing --autonomous (defaults posted for override), attaches UI mockups and current-app screenshots as PR evidence when a browser provider exists, applies full SDLC labels, and emits PR/spec markers for chaining into xez-auto-implement-spec. Use for "write a spec for X and open a PR", "spec this issue".
---

# Auto Write Spec (brief/issue → spec PR)

Run unattended: the user starts you and comes back to a **published spec PR** — the spec document, resolved-assumptions comment, and (for UI-facing features) mockups and screenshots attached as PR evidence. Composition, not reinvention: `xez-spec-writing --autonomous` writes the document, `xez-open-pr` ships it, the browser-provider descriptor captures visuals.
...
2. **Create an isolated worktree and spec branch.** Never run in the user's primary worktree. Branch `spec/${SLUG}` detached from `origin/$BASE_BRANCH`; record `CREATED_WORKTREE` so any worktree you created is cleaned up in a trap/finally. Full commands: `references/worktree-setup.md`. When the brief names a handoff file (a `— brief: <path>` suffix from `xez-brainstorm`), read it in the invoking checkout **before** creating the worktree and copy it in — a branch from `origin` does not contain it.

3. **Write the spec (autonomous).** Invoke the `xez-spec-writing` skill **verbatim, in `--autonomous` mode**, with the brief (or the issue title + body + relevant comments) as input. It writes `${SPECS_DIR}/{YYYY-MM-DD}-${SLUG}.md`, resolving any Open Questions per its Autonomous defaults rules into a `## Resolved assumptions (autonomous defaults)` section, and reports the resolved table back to you. A handoff brief's Resolved-unknowns table pre-answers Open Questions — autonomous defaults apply only to what it leaves open. Keep its output — steps 6 and 7 post it.
...
- Autonomous by default is this skill's only mode — a human who wants to answer the Open Questions should run `xez-spec-writing` directly.
...
- Token discipline: do not re-read the whole repo — `xez-spec-writing` step 1 already bounds context loading; reuse its findings.
