...
Two related things `xez-auto-fix-pr` runs: the **outer loop** that drives a PR to
...
1. **Conflicts, then review findings — `xez-auto-review-pr {prNumber} --autofix`**
...
3. **Verify UI — `xez-auto-qa-pr {prNumber}`** — only when the diff touches a
...
7. **Wait for CI — bounded**: **watch-run** on the new runs (or poll
   **get-pr-checks**) until they settle, for at most `CI_MAX_WAIT_MINUTES`
   ...
   without CI — the hand-off to `xez-approve-merge-pr` still requires genuinely green
   required checks.
...
tracked follow-up (via `xez-followup-issue-from-pr` or **create-issue**): test name,
...
- `xez-auto-review-pr` returns approvable — no conflicts and no actionable blockers.
