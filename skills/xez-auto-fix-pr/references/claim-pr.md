...
## xez-auto-fix-pr specifics

- **Outer lock.** This skill claims `{prNumber}` once (assignee via **assign-pr** + `in-progress` + 🤖 claim comment) and holds the **outer** claim for the entire run. The sub-skills it invokes (`xez-auto-review-pr`, `xez-auto-qa-pr`) will see `$CURRENT_USER` already owns the PR and treat their own claim as re-entry — that is expected, do not fight it.
- **Fetch fields.** Read the PR via **get-pr** with fields `number,title,state,headRefName,headRepositoryOwner,baseRefName,labels,assignees,isDraft,author,url`. Stop if the PR is already merged or closed.
- **Release on every exit.** Register a `trap`/finally at claim time that releases the outer lock (label removal via the guard, **unassign-pr** where the convention expects it) on any exit path — success, blocker, or crash.
- **`--ci-only` modes.** PR mode (a `{prNumber}`, or a `--branch` for which **search-prs** finds an open PR — then switch to PR mode on it) claims normally. Plain-branch mode takes **no** claim: there is nothing to lock, and no PR comment or label mutation happens — the branch and the run summary are the deliverable.
