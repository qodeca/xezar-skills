...
The step-3 procedure `xez-auto-fix-pr` runs before any review or CI work, so the
...
3. **Conflicts**: resolve trivial ones directly (import ordering, changelog/lock
   ...
   let the `xez-auto-review-pr` autofix flow (step 4) own conflict resolution, since
...
let step 4's `xez-auto-review-pr` run its **fork carry-forward flow**: it bases a
...
supersede/credit section). From that point, `{prNumber}` in the rest of `xez-auto-fix-pr`
