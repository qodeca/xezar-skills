...
```markdown
🚀 `xez-auto-fix-pr`: {merge-ready | needs human | blocked after {n} iterations} — {decisive reason and next action}.
...
```markdown
🧪 `xez-auto-fix-pr --ci-only`: {all required checks green | still red | unverified} — {cause fixed or remaining failure}.
