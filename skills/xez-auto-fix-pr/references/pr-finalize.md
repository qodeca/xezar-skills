...
This skill drives an **existing** PR to merge-ready; it never opens one, so there is no duplicate to guard against. The only PR ever opened in its orbit is the fork carry-forward **replacement** PR, opened by the delegated `xez-auto-review-pr` flow (see the fork section below) — from that point `{prNumber}` refers to the replacement, and every later mutation targets that same PR via **get-pr** / **comment-pr**.
...
- **Label commentary**: reflect every label change in the single idempotent `` 🤖 `xez-auto-fix-pr` — 🏷️ label rationale `` comment (one label per line with its emoji and a full-sentence reason, updated in place via **update-comment**; format and emoji map: the shared contract in `xez-open-pr`'s label normalization) — never one comment per change.
- **QA meta**: keep `needs-qa` when user-facing behavior changed and `QA_GATE` is on. `xez-auto-qa-pr` runs here in evidence-only mode, so it attaches screenshots but sets **no** QA labels; this skill **never** adds `qa-approved` or `qa-self-verified` — the QA gate and a QA reviewer own that (a human can opt into the self-QA sign-off separately). When the gate is on, a `needs-qa` PR stays unmergeable until signed off.
...
When ready, give `xez-approve-merge-pr {prNumber}` as the next command and state
...
Chained consumers (`xez-approve-merge-pr`, orchestration scripts) parse these exact text markers — never rename, translate, or decorate them.
...
- For each such finding, invoke `xez-followup-issue-from-pr` verbatim with the PR link (or, when the finding lives in a specific review comment, that comment's link) so its comment-mode extracts the actionable ask and assigns the issue to the right person (the comment's @-mention, else the PR author).
