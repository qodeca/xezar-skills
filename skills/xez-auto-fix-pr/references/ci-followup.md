...
## Disclosing pending CI in the summary comment

When a required check was still `PENDING` at the moment this skill reported, the run
summary comment MUST carry the disclosure as its own short paragraph, so the PR is
...
## Where this fires in the loop

The outer loop's CI stage is reached only once the branch has **no conflicts and no
outstanding review findings** — `xez-auto-review-pr --autofix` owns both, in that
order, and this skill never re-implements them. Once the last fix is pushed and the
loop has nothing left to do but watch, run step 6 in full — labels normalized, the
draft promoted, the summary comment posted, the outer lock swapped to
`ci-monitoring` — and only then wait. A merge-ready verdict still requires green
required checks: the hand-off to `xez-approve-merge-pr` says what CI actually did, and
that skill refuses the merge until the checks are genuinely green.
