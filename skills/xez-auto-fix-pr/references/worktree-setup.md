...
Detailed procedure for step 2 (create) and step 6 (cleanup) of `xez-auto-fix-pr`. Never run in the user's primary worktree.
...
WORKTREE_PARENT="$REPO_ROOT/.local/tmp/xez-auto-fix-pr"
CREATED_WORKTREE=0
...
- If the loop switches to a fork carry-forward replacement PR (see `references/base-merge.md`), the carry branch created by `xez-auto-review-pr` becomes the checked-out branch for the remaining cycles.
