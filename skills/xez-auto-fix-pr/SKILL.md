---
name: xez-auto-fix-pr
description: Drive an open PR to merge-ready from its number — merges the latest base, then loops review-autofix (xez-auto-review-pr), built-in CI stabilization, and UI verification (xez-auto-qa-pr) until approvable, green, and QA-evidenced. A --ci-only mode drives just CI green on a PR or a plain branch. Files follow-up issues for nits, normalizes labels, hands off to xez-approve-merge-pr — never merges itself. Use for "get PR 123 merge-ready".
---
...
stays with `xez-approve-merge-pr` / `xez-merge-buddy` behind the QA gate.

This skill is an **orchestrator**: it holds the outer claim and coordinates
`xez-auto-review-pr` (review + autofix + conflict/fork handling), `xez-auto-qa-pr`
(UI QA), and `xez-followup-issue-from-pr` (nit follow-ups), plus the **built-in CI
...
`xez-auto-fix-issue` (issue-side chain).

A **`--ci-only` mode** drives just CI green — on a PR (`xez-auto-fix-pr 123
--ci-only`) or on a plain branch with no PR yet (`xez-auto-fix-pr --ci-only --branch
...
This skill consumes a `{prNumber}` (the `PR:` reference line a PR-producing skill emitted) and drives that existing PR to merge-ready; it never opens a PR, so there is no duplicate to guard against (a fork carry-forward replacement is opened by the delegated `xez-auto-review-pr` flow, not here). It ends by reporting the `PR:` / `Issue:` chaining reference lines so the next skill in a chain can consume them, and hands the merge-ready PR to `xez-approve-merge-pr` (it never merges itself). Companion skills, each invoked verbatim: `xez-auto-review-pr` (review + autofix + conflict/fork handling), `xez-auto-qa-pr` (UI QA), `xez-followup-issue-from-pr` (nit follow-ups), and `xez-approve-merge-pr` (the merge hand-off) — a missing one stops the run and names the skill to install. CI stabilization is built in (`references/stabilize-ci.md`), not a delegated skill.

## Workflow

**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-auto-fix-pr.md` when present; safety rules still win.
...
- Orchestrate, don't reinvent: delegate review/autofix/conflict/fork handling to `xez-auto-review-pr`, UI QA to `xez-auto-qa-pr`, and nit follow-ups to `xez-followup-issue-from-pr`; invoke each verbatim and pass its outputs on. CI stabilization is built in (`references/stabilize-ci.md`) — follow that procedure rather than re-deriving it.
...
- **Conflicts first, then findings, then CI** — the loop's stage order is mandatory, and both earlier stages are delegated to `xez-auto-review-pr --autofix` rather than re-implemented here. CI is never stabilized on a branch that is still conflicted or still carries actionable review findings.
...
- **Fork supersede/credit**: when the review step carries a fork PR forward into a replacement PR, preserve the `Supersedes #{prNumber}` line, credit the original author, and reassign the replacement to them — per `xez-auto-review-pr`'s fork flow and the Supersede Credit Rule checks in `references/pr-finalize.md`.
...
- **Never merges, never fakes QA**: this skill leaves the PR merge-ready and hands off; it never squash-merges and never adds `qa-approved` (the QA gate and `xez-approve-merge-pr` own that). When the QA gate is on, a `needs-qa` PR stays unmergeable until a QA reviewer signs off.
