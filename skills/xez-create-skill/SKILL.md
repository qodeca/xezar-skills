---
name: xez-create-skill
description: Author a new pipeline skill from a brief, or split an oversized SKILL.md into layered references/ files — conservatively, behind the lint + completeness gate. Knows the layering philosophy, lint invariants, tracker abstraction, and the cross-skill contract, so output matches house conventions. Use for "create a skill for…", "new xez-skill", "split this skill into references".
---

# Create Skill

Author or refactor pipeline skills so they match this repo's conventions: a thin
...
**ALWAYS check first:** Apply `.xezar/pipeline/overrides/xez-create-skill.md` when present; safety rules still win.
...
0. **Agentic setup** — follow `references/agentic-setup.md`: load the repo's
   ...
   `references/philosophy.md` — the layering philosophy, the agent instruction files)
