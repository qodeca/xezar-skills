...
The hard, repo-specific constraints `xez-create-skill` bakes into every skill it
authors or splits. These are what make a skill "pipeline-shaped" rather than generic.
...
Skills name operations; the descriptor at `.xezar/pipeline/trackers/<tracker>.md` maps them
...
- **Config load**: the standard `.xezar/pipeline/config.json` snippet from the
  `xez-setup-agent-pipeline` skill, resolving `BASE_BRANCH`, `LABELS_ENABLED`,
  `QA_GATE`, `TRACKER`/`TRACKER_FILE`, and `validation.commands`.
- **Repo-local extension check**: after loading config, check for
  `.xezar/pipeline/overrides/<name>.md` and treat it as configuration that can add repo
...
- **Isolated worktree**: detect an existing linked worktree, otherwise create a
  temporary one under `.local/tmp/<skill>/`, and clean up only what this run
