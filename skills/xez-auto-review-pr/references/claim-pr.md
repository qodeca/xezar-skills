...
| In progress, current user owns the lock | — | Re-entry (incl. a chain hand-off lock from `xez-open-pr --handoff` or a flow runner's outer claim): post the take-over comment naming this skill **before any review work**, then proceed without re-claiming |
...
```text
🤖 `xez-auto-review-pr` started by @{CURRENT_USER} at {timestamp}. Other auto-skills will skip this PR until the lock is released.
```
...
```text
🤖 `xez-auto-review-pr` completed: {VERDICT}. Lock released.
```
...
```text
🤖 `xez-auto-review-pr` completed: {VERDICT}. Lock retained — chain continues.
```

The chain's driving skill (e.g. `xez-auto-fix-issue`) releases the lock at the end of its run.
