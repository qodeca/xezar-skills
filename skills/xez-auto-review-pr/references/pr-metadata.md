...
Detailed procedure for steps 2–3 of `xez-auto-review-pr`.
...
fixing chains (`xez-auto-fix-pr`, `xez-auto-fix-issue`) exist to clear it. Collect it from all three
...
**Severity.** Map the reviewer's own wording onto the `xez-code-review` scale — "must" / "required" /
