...
| you want | use |
|---|---|
| the full setup with a Claude Code leader running the work | [`xez-onboard-opinionated`](docs/skills/xez-onboard-opinionated.md) |
| a PR pipeline, no leader, any coding agent | [`xez-setup-agent-pipeline`](docs/skills/xez-setup-agent-pipeline.md) |
| a light setup for any kind of work — software, campaign, research, other | [`xez-onboard`](docs/skills/xez-onboard.md) |

Nothing here is deprecated. `xez-onboard-opinionated` is deliberately narrow — Claude Code only, GitHub only, clean projects only — and says so before it touches anything; the other two stay general. `xez-onboard` inspects first and previews only useful project files; no pipeline or leader is required.
...
It inspects your repo (default branch, validation scripts, GitHub labels), asks a few questions, writes `.xezar/pipeline/config.json`, and proposes project guidance — a ticket-flow doc such as `SDLC.md`, for one — only where you asked for it and the repository shows a need. It shows the content before writing and never creates a process just because a file is missing. The software pipeline skills read the config; onboarding does not require it. The standalone `xez-issue-create` skill reads existing project guidance and works without setup.
...
**Naming convention:** the `xez-auto-*` prefix means **autonomous and non-interactive** — hand these a brief, an issue, or nothing at all and they run end-to-end without supervision: they claim their work with the `in-progress` lock so concurrent agents back off, work in isolated worktrees so your checkout stays untouched, run the validation gate, self-review, make the recommended most-reversible call themselves (documented for override) instead of stopping to ask, and finish with a PR, a review verdict, or a reconciled tracker. Safe to run on a schedule or in CI. Three skills are autonomous without the prefix, on purpose: [`xez-pr-autopilot`](docs/skills/xez-pr-autopilot.md) takes a PR number and dispatches, and [`xez-review-prs`](docs/skills/xez-review-prs.md) and [`xez-close-fixed-issues`](docs/skills/xez-close-fixed-issues.md) are sweeps that ask nothing. Every other skill is interactive: it acts once, may ask you questions, reports, and hands control back.
...
| [`xez-auto-fix-issue`](docs/skills/xez-auto-fix-issue.md) | The single issue-to-PR entry point: takes an issue, or a plain description it first files as one, classifies it, then routes. A bug drives the autofix chain — triage gate, root-cause analysis, minimal fix with regression tests, a ready labeled PR, autofix review loop. A feature request takes the feature route — claims the issue, resolves its spec (autonomously written via [`xez-auto-write-spec`](docs/skills/xez-auto-write-spec.md) when none exists, implemented via [`xez-auto-implement-spec`](docs/skills/xez-auto-implement-spec.md)), and verifies the contract on the same PR — reviewed, UI-verified, fully labeled. For a spec without implementation, run `xez-auto-write-spec` directly. Stops cleanly when the issue is already solved or claimed. |
...
Interactive helpers — the other half of the naming convention: they act once, may ask you questions along the way, report, and hand control back to you.

| Skill | What it does |
|---|---|
| [`xez-setup-agent-pipeline`](docs/skills/xez-setup-agent-pipeline.md) | One-per-repo configurator. Inspects the repository, asks a few questions, writes `.xezar/pipeline/config.json`, installs tracker and browser-provider descriptors, and proposes project guidance (a process doc, an agent-instructions starter) only when you ask for it — shown before it is written, never created merely because a file is absent. Verifies cross-skill coverage: if an installed skill references one that isn't installed, it prints the exact `npx skills add` command to fix it. |
...
| `/xez-prepare-issue "Bulk-archive orders from the grid"` | dedupe search, [`xez-auto-write-spec`](docs/skills/xez-auto-write-spec.md) (when a feature needs a spec and none exists — it opens a spec PR) | one well-formed issue with SDLC labels, a linked spec or step-by-step guidance |
...
A Rust repo puts `cargo test` and `cargo clippy` in `validation.commands`; a Go repo puts `go test ./...`. Skills run whatever you configure and treat any non-zero exit as a gate failure. A software pipeline skill invoked in a repo without the config runs [`xez-setup-agent-pipeline`](docs/skills/xez-setup-agent-pipeline.md) first **only when setting up a software pipeline is itself authorized** — a missing config alone is not permission. `--defaults` picks detected values inside that permission and grants none of its own. Otherwise the skill reports the missing prerequisite and stops, rather than creating a process to make the task fit.
...
- **Project docs, when you want them** — `SDLC.md` (the process doc), `CODE_REVIEW.md` (review rules, auto-applied by [`xez-code-review`](docs/skills/xez-code-review.md)), `BACKWARD_COMPATIBILITY.md` (protected contract surfaces — reviews flag violations, implementations warn you), and an `AGENTS.md` starter with a task-routing table. [`xez-setup-agent-pipeline`](docs/skills/xez-setup-agent-pipeline.md) can propose any of them, derived from your repository — but none is required, the filenames are yours to choose, and a missing document is not by itself a reason to write one. It shows the content first; existing docs are honored as-is.
...
Every PR carries at most one pipeline label (`review`, `changes-requested`, `merge-queue`, ...) plus additive category, meta, priority, and risk labels; priority says how urgent the work is, risk says how dangerous the change is to ship. The full taxonomy, and whether to use labels at all, lives in the config; [`xez-setup-agent-pipeline`](docs/skills/xez-setup-agent-pipeline.md) documents every group. It creates only the missing labels you authorize; with no established taxonomy it leaves labels off or proposes choices, and `--defaults` creates none on its own.
