...
assert.match(prepare, /BROWSER_FILE="\.xezar\/pipeline\/browsers\/\$\{BROWSER_PROVIDER\}\.md"/);
...
for (const consumer of [qaPr, integration]) {
  assert.match(consumer, /\.xezar\/pipeline\/browsers\/<provider>\.md|\.xezar\/pipeline\/browsers\/\$\{BROWSER_PROVIDER\}\.md/);
