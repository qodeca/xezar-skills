...
const MATRIX = [
  ...
  {
    capability: "suppresses package lifecycle scripts on restore",
    provider: `${refs}/toolchains/yarn.md`,
    expect: /yarn install --frozen-lockfile --non-interactive --ignore-scripts/,
    why: "Yarn 1 runs preinstall/install/postinstall from every transitive package, as npm does",
  },
  {
    capability: "suppresses package lifecycle scripts on restore",
    provider: `${refs}/toolchains/dotnet.md`,
    expect: /RESTORE_SCRIPTS_SUPPRESSED=not-applicable/,
    why: "NuGet PackageReference restore runs no package scripts; this must be recorded, not left blank",
  },
  {
    capability: "restores without rewriting the lockfile",
    provider: `${refs}/toolchains/yarn.md`,
    expect: /--frozen-lockfile/,
    why: "a plain Yarn 1 install rewrites yarn.lock",
  },
  {
    capability: "restores without rewriting the lockfile",
    provider: `${refs}/toolchains/dotnet.md`,
    expect: /--locked-mode/,
    why: "without locked mode NuGet updates packages.lock.json instead of failing",
  },
  {
    capability: "refuses a same-named tool it does not describe",
    provider: `${refs}/toolchains/yarn.md`,
    expect: /this descriptor is Yarn 1 only/,
    why: "Yarn 2 and later have other flags and another lockfile; guessing would run the wrong tool",
  },
  {
    capability: "ships `outdated` with the base tool",
    provider: `${refs}/toolchains/yarn.md`,
    expect: /yarn outdated --json/,
    why: "Yarn 1 ships it, and it exits 1 when something is behind",
  },
  {
    capability: "ships `outdated` with the base tool",
    provider: `${refs}/toolchains/dotnet.md`,
    expect: /exits \*\*0 whether or not anything is out of date\*\*/,
    why: "the dotnet trap is the opposite of npm's: the status must come from the parsed output",
  },
  {
    capability: "has a build step at all",
    provider: `${refs}/toolchains/yarn.md`,
  ...
  },
  {
    ...
    provider: `${refs}/toolchains/dotnet.md`,
    expect: /never `not-applicable`/,
    why: "a solution always builds; saying so is part of the contract",
  },
  {
    capability: "pins an exact version on update",
    provider: `${refs}/toolchains/yarn.md`,
  ...
  },
  {
    ...
    why: "a PackageReference names one version; the update must write the one decided",
  },
  {
    capability: "reports how far an update spread",
    provider: `${refs}/toolchains/dotnet.md`,
    expect: /with no lock\nfiles it is `unknown`, not `0`/,
    why: "without packages.lock.json the spread cannot be measured, and zero would be a guess",
  },
  {
