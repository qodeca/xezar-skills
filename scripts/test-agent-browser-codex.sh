...
cleanup() {
  if [ -f "$FIXTURE/.xezar/pipeline/scripts/test-env-down.sh" ]; then
    (cd "$FIXTURE" && sh .xezar/pipeline/scripts/test-env-down.sh) >/dev/null 2>&1 || true
  fi
...
mkdir -p "$FIXTURE/.agents/skills" "$FIXTURE/.xezar/pipeline/browsers" "$LOG_DIR"
for skill in xez-setup-agent-pipeline xez-prepare-test-env xez-auto-qa-pr xez-integration-tests; do
  ln -s "$ROOT/skills/$skill" "$FIXTURE/.agents/skills/$skill"
done
cp "$ROOT/skills/xez-setup-agent-pipeline/references/browsers/agent-browser.md" "$FIXTURE/.xezar/pipeline/browsers/agent-browser.md"
...
run_codex prepare 'Use $xez-prepare-test-env with --mode dev --no-ephemeral and --browser-provider agent-browser. This is an isolated test fixture; proceed without questions. The app command and port contract are in AGENTS.md. Finish only after the generated entrypoint passes cold and warm runs and test-env.json reports a healthy agent-browser provider.'

test -s "$FIXTURE/.local/qa/test-env.json"
node -e 'const d=require(process.argv[1]); if(d.status!=="running"||d.browser?.provider!=="agent-browser"||d.browser?.installed!==true) process.exit(1)' "$FIXTURE/.local/qa/test-env.json"

run_codex verify 'Use $xez-auto-qa-pr in local evidence-only mode with --keep-env. Verify the uncommitted title and button-text change through the configured agent-browser provider. Proceed without questions. Require a PASS report and a non-empty PNG under .local/qa; do not modify index.html.'

REPORT=$(find "$FIXTURE/.local/qa" -path '*/report.json' -type f | head -n 1)
test -n "$REPORT"
