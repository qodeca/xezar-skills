...
// Contract test for the xez-pipeline-retro classifier, the collection's first
...
{
  const once = classify([merged(1, [opener("xez-auto-review-pr", "2026-08-01T10:05:00Z")])]);
  assert.equal(once.pullRequests[0].runs["xez-auto-review-pr"], 1, "single naming counts one run");
  assert.equal(once.pullRequests[0].class, "clean");

  const twice = classify([merged(2, [doubleNamingOpener("xez-auto-review-pr", "2026-08-01T10:05:00Z")])]);
  assert.equal(twice.pullRequests[0].runs["xez-auto-review-pr"], 1, "double naming still counts one run");
...
// Two genuine opener lines are still two runs — the dedup is per line, not global.
{
  const result = classify([
    merged(3, [
      opener("xez-auto-review-pr", "2026-08-01T10:05:00Z"),
      opener("xez-auto-review-pr", "2026-08-01T11:05:00Z"),
    ], { reviews: [{ state: "CHANGES_REQUESTED", body: "needs work" }] }),
  ]);
  assert.equal(result.pullRequests[0].runs["xez-auto-review-pr"], 2);
...
{
  const result = classify([
    merged(4, [{ createdAt: "2026-08-01T10:05:00Z", body: "> 🤖 `xez-auto-review-pr` started by @bot" }]),
  ]);
  ...
  const result = classify([
    {
      ...
      labels: [],
      reviews: [],
      comments: [opener("xez-auto-review-pr", "2026-08-01T10:05:00Z")],
    },
  ]);
  assert.equal(result.pullRequests[0].class, "hard recovery");
...
{
  const result = classify([
    merged(6, [opener("xez-auto-continue-pr-loop", "2026-08-01T10:05:00Z")]),
  ]);
...
{
  const result = classify([
    merged(7, [opener("xez-auto-fix-pr", "2026-08-01T10:05:00Z")]),
  ]);
  ...
  const result = classify([
    {
      ...
      reviews: [],
      comments: [opener("xez-auto-fix-pr", "2026-08-01T10:05:00Z")],
    },
  ]);
...
{
  const result = classify([
    merged(10, [opener("xez-auto-review-pr", "2026-08-01T10:05:00Z")], {
...
{
  const result = classify([
    merged(12, [{ body: "🤖 `xez-auto-review-pr` posted something without a timestamp" }]),
  ]);
