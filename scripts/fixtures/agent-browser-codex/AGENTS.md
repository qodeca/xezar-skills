...
- Generate `.xezar/pipeline/scripts/test-env-up.sh` and `test-env-down.sh` around that command.
- Use a free port and bind to `127.0.0.1`.
- The ready page has an `h1` and one button. No credentials or backing services
  exist.
- Browser evidence must use the configured `.xezar/pipeline/browsers/agent-browser.md`.
